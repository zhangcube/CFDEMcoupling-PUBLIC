...
defineTypeNameAndDebug(BenyahiaDrag, 0);
...
void BenyahiaDrag::setForce() const
{
    ...
    for(int index = 0;index <  particleCloud_.numberOfParticles(); index++)
    {
            ...
            if (cellI > -1) // particle Found
            {
                if(forceSubM(0).interpolation())
                {
                    ...
                    if(voidfraction<0.05) voidfraction = 0.05;
                }else
                {
                ...
                if (magUr > 0)
                {
                    ...
                    if(volumefraction > 0.01 && volumefraction < 0.4)
                    {
                        F0 = (1. - w) * (1. + 3. * sqrt(volumefraction / 2.) + (135. / 64.) * volumefraction * log(volumefraction)
                             ...
                             ) + w * 10. * volumefraction / (voidfraction * voidfraction * voidfraction);
                    } else {
                    ...
                    if(volumefraction < 0.4)
                    {
                        ...
                        F2 = 10.*volumefraction/(voidfraction*voidfraction*voidfraction);
                    }
                    if(volumefraction < 0.0953)
                    {
                        F3 = 0.9351 * volumefraction + 0.03667;
                    } else {
                        F3 = 0.0673 + 0.212 * volumefraction +0.0232/pow(voidfraction,5.);
                    }
                    ...
                    betaP = 18.*nuf*rho/(ds*ds)*voidfraction*voidfraction*F;
                    ...
                    forceSubM(0).scaleCoeff(dragCoefficient,dParcel,Rep,index);
            ...
            forceSubM(0).partToArray(index,drag,dragExplicit,Ufluid,dragCoefficient,vector::zero,voidfraction,alphawater);
