...
defineTypeNameAndDebug(BenyasemiDrag, 0);
...
void BenyasemiDrag::setForce() const
{
    ...
    scalar voidfraction(1);scalar voidfractionn(1);//added by gp
    vector Ufluid(0,0,0);vector Ufluidd(0,0,0);//added by gp
    vector drag(0,0,0);
    vector dragExplicit(0,0,0);
  	scalar dragCoefficient(0);
    label cellI=0;label i;//added by gp
    ...
    scalar sumUCoefficient(0); scalar distance(0);//added by gp
    scalar coefficient(0);//added by gp
    scalar sumVol(0);//added by gp
    ...
    {
            cellI = particleCloud_.cellIDs()[index][0];i = particleCloud_.cellIDs()[index][0];//added by gp
            drag = vector(0,0,0);
            ...
            Vs = 0;
            Ufluid =vector(0,0,0);
            voidfraction=0;voidfractionn=0;//added by gp
            sumUCoefficient=0;distance=0;coefficient=0;sumVol=0;//added by gp          
...
///////////////////////////////////////////////////////
                ...
                volumefraction = max(SMALL,min(1-SMALL,1-voidfractionn));
                ...
                if (magUr > 0)
                {
                    ...
                    scalar F3=0.;
                    scalar w=0.;
                    w=exp(-10.*(0.4-volumefraction)/volumefraction);
                    if(volumefraction > 0.01 && volumefraction < 0.4)
                    {
                        F0 = (1. - w) * (1. + 3. * sqrt(volumefraction / 2.) + (135. / 64.) * volumefraction * log(volumefraction)
                              + 17.14 * volumefraction
                             )/
                             (1. + 0.681 * volumefraction - 8.48 * sqr(volumefraction)
                              + 8.16 * volumefraction * volumefraction * volumefraction
                             ) + w * 10. * volumefraction / (voidfractionn * voidfractionn * voidfractionn);
                    } else {
                        F0 = 10. * volumefraction / (voidfractionn * voidfractionn * voidfractionn);
                    }
                    if(volumefraction > 0.01 && volumefraction <= 0.1)
                    {
                        F1 = sqrt(2./(volumefraction))/40.;
                    } else {
                        F1 = 0.11+0.00051*exp(11.6*volumefraction);
                    }
                    if(volumefraction < 0.4)
                    {
                        F2 = (1.-w) * (1. + 3.*sqrt((volumefraction)/2.) + (135./64.)*volumefraction*log(volumefraction)
                              + 17.89*volumefraction
                             )/
                             (1.+0.681*volumefraction-11.03*sqr(volumefraction)
                              +15.41*volumefraction*volumefraction*volumefraction
                             ) + w*10.*volumefraction/(voidfractionn*voidfractionn*voidfractionn);
                    } else {
                        F2 = 10.*volumefraction/(voidfractionn*voidfractionn*voidfractionn);
                    }
                    if(volumefraction < 0.0953)
                    {
                        F3 = 0.9351 * volumefraction + 0.03667;
                    } else {
                        F3 = 0.0673 + 0.212 * volumefraction +0.0232/pow(voidfractionn,5.);
                    }
                    ...
                    scalar Repc1 =(F2 - 1.)/(3./8.-F3);
                    scalar Repc2 =(F3+sqrt(F3*F3-4.*F1*(F0-F2)))/(2.*F1);
                    scalar F =0.;
                    if((volumefraction <= 0.01) && (Rep <= Repc1))
                    {
                        F = 1. + 3./8.*Rep;
                    } else if((volumefraction > 0.01) && (Rep <= Repc2))
                    { 
                        F = F0+F1*Rep*Rep;
                    } else {
                        F = F2+F3*Rep;
                    ...
                    betaP = 18.*nuf*rho/(ds*ds)*voidfractionn*voidfractionn*F;
