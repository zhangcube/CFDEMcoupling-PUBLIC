...
:
    ...
    useShearInduced_(false),
    useSpinInduced_(false),
    combineShearSpin_(false)
{
    // read switches
    if (propsDict_.found("useShearInduced"))
        useShearInduced_=Switch(propsDict_.lookup("useShearInduced"));
    if (propsDict_.found("useSpinInduced"))
        useSpinInduced_=Switch(propsDict_.lookup("useSpinInduced"));
    if (propsDict_.found("combineShearSpin"))
        combineShearSpin_=Switch(propsDict_.lookup("combineShearSpin"));
