...
defineTypeNameAndDebug(BrownLawlerDrag, 0);
...
BrownLawlerDrag::BrownLawlerDrag
(
...
void BrownLawlerDrag::setForce() const
{
    ...
    for(int index = 0;index <  particleCloud_.numberOfParticles(); index++)
    {
        ...
        if (cellI > -1) // particle Found
        {
            ...
            ds = particleCloud_.cfdemCloud::diameter(index);
            dParcel = ds;
            ...
            if (magUr > 0 && voidfraction > 0)
            {
                // calc particle Re Nr
                Rep = ds*voidfraction*magUr/(nuf+SMALL);

                // calc fluid drag Coeff
                Cd = 24.0/Rep*(1.0+0.15*pow(Rep,0.681))+0.407/(1+8710/Rep);
